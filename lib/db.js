...
// Same tagged-template call shape the routes use (`sql\`SELECT ...\``), lazily
// resolving the connection on first use so importing this module never
// crashes when the env var isn't set yet. Neon's driver resolves to a bare
// array of rows; wrap it as `{ rows }` so callers can destructure it.
export async function sql(strings, ...values) {
  const rows = await getNeonSql()(strings, ...values);
  return { rows };
}
...
export function ensureSchema() {
  if (!ready) {
    ready = (async () => {
      ...
      ready = null; // don't cache a failure — let the next request retry
      throw e;
    });
  }
  return ready;
